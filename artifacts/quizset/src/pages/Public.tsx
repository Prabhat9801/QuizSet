...
export function Login(){const [email,setEmail]=useState('');const [password,setPassword]=useState('');const [showPassword,setShowPassword]=useState(false);const [forgot,setForgot]=useState(false);const [error,setError]=useState('');const [loc,setLoc]=useLocation();const {login,toast,sessionKicked,clearSessionKicked}=useApp();const quick=(e:string,p:string)=>{setEmail(e);setPassword(p);submit(e,p)};
